...
import { api } from './api';
import type { ChatAnswer, ChatExchange, ChatFeedback, ChatShareStatus, Concept, ContributionSummary, DemoUser, Obligation, Organization, OrganizationProfile, Source, SupervisionTheme, TaskStatus, UserReportedRequirement } from './domain/types';
...
function Overview({ organization, organizationMutedBefore, obligations, catalogObligations, allObligations, sources, supervisionThemes, selectedObligation, selectedOccurrenceDate, setSelectedObligationId, setSelectedOccurrenceDate, calendarStart, setCalendarStart, calendarMode, setCalendarMode, statusFilter, setStatusFilter, visibilityFilter, setVisibilityFilter, onTaskChanged, onOrganizationViewChanged, onContributionChanged }: { organization: Organization; organizationMutedBefore: string; obligations: Obligation[]; catalogObligations: Obligation[]; allObligations: Obligation[]; sources: Source[]; supervisionThemes: SupervisionTheme[]; selectedObligation: Obligation | null; selectedOccurrenceDate: string | null; setSelectedObligationId: (id: string | null) => void; setSelectedOccurrenceDate: (date: string | null) => void; calendarStart: Date; setCalendarStart: (date: Date) => void; calendarMode: 'year' | 'list'; setCalendarMode: (mode: 'year' | 'list') => void; statusFilter: 'all' | TaskStatus; setStatusFilter: (value: 'all' | TaskStatus) => void; visibilityFilter: 'visible' | 'hidden' | 'muted' | 'all'; setVisibilityFilter: (value: 'visible' | 'hidden' | 'muted' | 'all') => void; onTaskChanged: (nextSelectedId?: string | null) => void; onOrganizationViewChanged: () => void; onContributionChanged: (points?: number, action?: string) => Promise<void> }) {
...
function SupervisionWorkspace({ organization, themes, obligations, sources, onSelectObligation, onAskLos }: { organization: Organization; themes: SupervisionTheme[]; obligations: Obligation[]; sources: Source[]; onSelectObligation: (id: string) => void; onAskLos: (prompt: string) => void }) {
...
function ObligationGuidance({ obligation }: { obligation: Obligation }) {
  ...
  return <section className="detail-section obligation-guidance"><strong>Mer om rapporteringen</strong>{info.targetAudience && <div><small>Hvem gjelder dette for?</small><p>{info.targetAudience}</p></div>}{info.usageContexts.length > 0 && <div><small>Når brukes den?</small><p>{info.usageContexts.join(' · ')}</p></div>}{info.usageNotes.length > 0 && <div><small>Hva beskriver registeret?</small><p>{info.usageNotes.join(' ')}</p></div>}{info.dataSources.length > 0 && <div><small>Opplysninger som kan inngå</small><p>{info.dataSources.join(' · ')}</p></div>}<p className="guidance-disclaimer">Dette er utdypende registerinformasjon. Se kildene under for gjeldende veiledning og innsending.</p></section>;
}
