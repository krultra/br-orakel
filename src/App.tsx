...
function App() {
  ...
  const filteredObligations = visibilityFilter === 'all'
    ? filteredByStatus
    : filteredByStatus.filter((item) => visibilityFilter === 'hidden' ? item.isHidden === true : visibilityFilter === 'muted' ? item.isMuted === true : item.isHidden !== true && item.isMuted !== true);
  const activeObligations = filteredObligations.filter((item) => item.isActivated === true);
...
function NoDatePanel({ items, selectedId, onSelect }: { items: Array<{ item: Obligation; date?: string }>; selectedId?: string; onSelect: (id: string, date?: string) => void }) {
  ...
  return <div className="month-cell no-date-panel has-items"><div className="no-date-heading"><div><span className="month-label">Uten fast frist</span><small>{filtered.length} av {items.length} oppgaver</small></div><span className="no-date-hint">Søk i katalogen og velg en oppgave for å legge den i arbeidslisten</span></div><div className="no-date-controls"><Textfield className="no-date-search" aria-label="Søk i oppgaver uten fast frist" placeholder="Søk etter hendelse eller oppgave" value={query} onChange={(event) => setQuery(event.target.value)} /><select value={eventFilter} onChange={(event) => setEventFilter(event.target.value)} aria-label="Filtrer oppgaver uten fast frist"><option value="all">Alle hendelser</option>{eventOptions.map((event) => <option value={event} key={event}>{event}</option>)}</select></div><div className="no-date-list">{filtered.length > 0 ? filtered.map(({ item }) => { const itemStatus = statusForDate(item.status, undefined, item.statusByDate); return <button key={item.id} className={`calendar-item ${statusClass[itemStatus]} ${selectedId === item.id ? 'is-selected' : ''}`} onClick={() => onSelect(item.id)}><span className="calendar-dot" /><span>{item.name}</span><small>{eventCategory(item)} · {statusLabels[itemStatus]}</small></button>; }) : <p className="no-date-empty">Ingen oppgaver matcher søket.</p>}</div></div>;
}
