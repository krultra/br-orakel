...
function App() {
  ...
  const loadOrganization = async (number = orgNumber, preserveSelection = false) => {
    setLoading(true);
    try {
      const [nextOrg, nextObligations, nextSources, nextViewPreference, nextProfile, nextSupervisionThemes, nextSupervisionNotices] = await Promise.all([api.organization(number), api.obligations(number), api.sources('', number), api.organizationViewPreference(number), api.organizationProfile(number), api.supervisionThemes(number), api.supervisionNotices(number)]);
      setOrganization(nextOrg); setOrganizationProfile(nextProfile); setOrganizationMutedBefore(nextViewPreference.mutedBefore ?? ''); setObligations(nextObligations); setSources(nextSources); setSupervisionThemes(nextSupervisionThemes); setSupervisionNotices(nextSupervisionNotices); setSelectedObligationId((current) => preserveSelection ? (current && nextObligations.some((item) => item.id === current) ? current : null) : null); if (!preserveSelection) setSelectedOccurrenceDate(null); setOrganizationSearchResults([]); setToast('Virksomhetsoversikten er oppdatert.');
    } catch (error) { setOrganization(null); setOrganizationProfile(null); setOrganizationMutedBefore(''); setSupervisionThemes([]); setSupervisionNotices([]); setToast(error instanceof Error ? error.message : 'Kunne ikke laste virksomheten.'); }
